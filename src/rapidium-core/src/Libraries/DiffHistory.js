//import { History } from '@rapCoreLibraries/HistorySchema';
// const History = require('./HistorySchema.ts').model;
const { History } = require('./HistorySchema.ts');
...
function saveDiffObject(currentObject, original, updated, opts, queryObject, type) {
    ...
    if (currentObject.last_modified_by) {
        userId = currentObject.last_modified_by;
    } else {
        if (collectionName == 'customer') {
            userId = currentObject.user_id
        }
    }
    let clientId = '';
    if(currentObject.client_id) {
        clientId = currentObject.client_id;
    } else {
        if (collectionName == 'customer') {
            clientId = currentObject.user_id
        }
    }

    
    // console.log(83, currentObject);
    // console.log(84, collectionName);
...
const saveDiffHistory = (queryObject, currentObject, opts, type) => {
...
const saveDelete = async (queryObject, opts, type) => {
    const collectionId = queryObject._conditions._id;
    const collectionName = queryObject.model.modelName;
    const user = queryObject.options.__user.last_modified_by;
    const diff = {};
    const history = new History({
        collectionId,
        collectionName,
...
const saveDiffs = (queryObject, opts, type) =>
    queryObject
