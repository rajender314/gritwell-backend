...
/**
 * class RecommendationMailer
 */
export default class RecommendationMailer extends MyMailer {
  /**
   * @param {mailObject} mailObject
   */
  async reccommendationEmail(mailObject:any) {
    const emailTemplate =
        await new EmailTemplate().getEmailTemplateFromDBName(
            'recommendations_updated_and_submitted',
        );
    if (emailTemplate) {
      let htmlText = '';
