...
import {VerifyAuthorizationToken}
  from '../../Commands/VerifyAuthorizationToken';
...
export default class MiddlewareController extends BaseController {
  ...
  * @param {next} next
  */
  async verifyAuthorizationToken(req: Request, res: Response, next:any) {
    const addCommand = new VerifyAuthorizationToken(req);
    await new BaseController().run(
        addCommand.path,
        addCommand,
        res,
